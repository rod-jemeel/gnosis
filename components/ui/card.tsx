...
function Card({
  ...
  return (
    <div
      data-slot="card"
      data-size={size}
      className={cn("border-foreground/90 bg-card text-card-foreground gap-4 overflow-hidden rounded-none border-2 py-4 text-xs/relaxed shadow-[5px_5px_0_0_var(--foreground)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-none *:[img:last-child]:rounded-none group/card flex flex-col", className)}
      {...props}
    />
  )
}
